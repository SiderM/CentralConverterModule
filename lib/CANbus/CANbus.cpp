...
void CANbus::begin(bool retransmission)
{
  ...
  if (retransmission)
  {
    n_pCanHandle->Init.AutoRetransmission = ENABLE;
  }
  else
  {
...
void CANbus::calculateBaudrate(CAN_HandleTypeDef *CanHandle, int baud)
{
  ...
  {
    if (lookupBaudrate(CanHandle, baud, BAUD_RATE_TABLE_48M))
      return;
  }
  else if (frequency == 45000000)
  {
    if (lookupBaudrate(CanHandle, baud, BAUD_RATE_TABLE_45M))
      return;
  }
...
void CANbus::enableLoopBack(bool yes)
{
  if (yes)
  {
    n_pCanHandle->Init.Mode = CAN_MODE_LOOPBACK;
  }
  else
  {
    n_pCanHandle->Init.Mode = CAN_MODE_NORMAL;
...
void CANbus::enableSilentMode(bool yes)
{
  if (yes)
  {
    n_pCanHandle->Init.Mode = CAN_MODE_SILENT;
  }
  else
  {
    n_pCanHandle->Init.Mode = CAN_MODE_NORMAL;
...
void CANbus::enableSilentLoopBack(bool yes)
{
  if (yes)
  {
    n_pCanHandle->Init.Mode = CAN_MODE_SILENT_LOOPBACK;
  }
  else
  {
    n_pCanHandle->Init.Mode = CAN_MODE_NORMAL;
